...
在主函数中，我们创建了一个名为 data 的 vector<int> 对象，并通过 ForwardFunction 分别将 data 和一个临时创建的 vector<int> 对象作为参数进行传递。因为 ForwardFunction 使用了完美转发，所以参数会根据原始参数的左值/右值属性被准确地转发给 OtherFunction。
